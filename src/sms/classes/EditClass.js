...
const EditClass = () => {
  ...
  const { id } = useParams();
  const { classes, updateClass } = useSchool();
  const cls = classes.find(c => c.id === parseInt(id));

  const [formData, setFormData] = useState({ name: '', sections: ['A'], capacity: 40 });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validate()) {
      updateClass(parseInt(id), formData);
      navigate('/classes');
    }
  };
    ...
    return (
      <main id="main" className="main">
        <div className="container-fluid">
          <div className="pagetitle"><h1>Class Not Found</h1></div>
        </div>
      </main>
    );
  }
                ...
                <div className="card-body">
                  ...
                  <form onSubmit={handleSubmit}>
                    ...
                    <div className="text-end">
                      ...
                      <button type="submit" className="btn btn-primary"><i className="bi bi-save"></i> Update Class</button>
                    </div>
                  </form>
                </div>
              </div>
